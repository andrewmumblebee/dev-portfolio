export default function About() {
  return (
    <section id="about" className="mt-24 container">
      <h2>About</h2>
      <p className="max-w-prose text-xl">
        I&apos;m a developer based in London. With over 5 years of experience,
        over my career I&apos;ve had a chance to work on a spectrum of web
        development.
        <br />I love working on frontend experiences, but enjoy working across
        the full stack. I have a passion for learning new things and keeping up
        to date with the latest technologies.
      </p>
    </section>
  )
}
